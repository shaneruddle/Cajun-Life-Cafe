...
urls.forEach(url => {
  ...
  // Create clean filename (hyphenated)
  let cleanFilename = decodedFilename.replace(/%20/g, ' ').replace(/\s+/g, '-');
  // Handle double hyphens if any
  cleanFilename = cleanFilename.replace(/-+/g, '-');
  
  const newPath = `/menu/${cleanFilename}`;
  
  mapping[originalFilename] = newPath;
  if (originalFilename !== decodedFilename) {
    mapping[decodedFilename] = newPath;
  }
});
