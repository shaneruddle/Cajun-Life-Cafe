...
- supplier: use the shop/brand name from the receipt header`
  ...
  // Queries system_logs for loyalty activity in the past 24h and emails a report.
  // Called by Cloud Scheduler at midnight ICT (17:00 UTC).
  // Also callable manually: POST /api/loyalty-digest (no body needed)
  ...
  async function sendLoyaltyDigest(): Promise<{ sent: boolean; entries: number; error?: string }> {
    ...
    if (!smtpHost || !smtpUser || !smtpPass) {
      return { sent: false, entries: 0, error: "SMTP not configured" };
    }

    // Query loyalty logs from past 24 hours
    ...
    let tableRows = "";
    if (logs.length === 0) {
      tableRows = `<tr><td colspan="4" style="text-align:center;color:#999;padding:20px;">No loyalty activity in the past 24 hours</td></tr>`;
    } else {
      for (const log of logs) {
        const time = new Date(log.timestamp).toLocaleTimeString("en-GB", { timeZone: "Asia/Bangkok", hour: "2-digit", minute: "2-digit" });
        const actionColor = log.action === "Receipt Redemption" ? "#e53e3e" : log.action === "Balance Loaded" || log.action === "Wallet Top Up" ? "#276749" : "#2b6cb0";
        tableRows += `
          <tr style="border-bottom:1px solid #eee;">
            <td style="padding:10px 12px;color:#666;white-space:nowrap;">${time}</td>
            <td style="padding:10px 12px;font-weight:600;color:${actionColor};white-space:nowrap;">${log.action}</td>
            <td style="padding:10px 12px;color:#333;font-size:13px;">${log.details}</td>
            <td style="padding:10px 12px;color:#999;font-size:12px;white-space:nowrap;">${log.userEmail || ""}</td>
          </tr>`;
      }
    }
...
<body style="margin:0;padding:0;background:#f7f7f7;font-family:Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f7f7f7;padding:30px 0;">
    <tr><td>
      <table width="620" cellpadding="0" cellspacing="0" style="margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
        <!-- Header -->
        <tr><td style="background:#1a1a1a;padding:24px 32px;">
          <p style="margin:0;color:#fff;font-size:20px;font-weight:700;">Cajun Life Cafe</p>
          <p style="margin:4px 0 0;color:#aaa;font-size:13px;">Loyalty Activity Digest — ${dateStr}</p>
        </td></tr>
        <!-- Summary -->
        <tr><td style="padding:24px 32px 0;">
          <p style="margin:0;color:#333;font-size:15px;">
            <strong>${logs.length} loyalty event${logs.length !== 1 ? "s" : ""}</strong> recorded in the past 24 hours.
          </p>
        </td></tr>
        <!-- Table -->
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border:1px solid #eee;border-radius:8px;overflow:hidden;">
            <thead>
              <tr style="background:#f9f9f9;">
                <th style="padding:10px 12px;text-align:left;color:#666;font-size:12px;text-transform:uppercase;letter-spacing:0.5px;">Time</th>
                <th style="padding:10px 12px;text-align:left;color:#666;font-size:12px;text-transform:uppercase;letter-spacing:0.5px;">Action</th>
                <th style="padding:10px 12px;text-align:left;color:#666;font-size:12px;text-transform:uppercase;letter-spacing:0.5px;">Details</th>
                <th style="padding:10px 12px;text-align:left;color:#666;font-size:12px;text-transform:uppercase;letter-spacing:0.5px;">Staff</th>
              </tr>
            </thead>
        ...
        <tr><td style="padding:16px 32px 24px;border-top:1px solid #eee;">
          <p style="margin:0;color:#aaa;font-size:12px;">This report is generated automatically each night at midnight ICT. For disputes or queries contact info@cajunlifecafe.com</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
    ...
    // Send via Gmail SMTP using nodemailer-style raw SMTP
    // We use the built-in node net/tls to avoid adding dependencies
    // Instead, use fetch to a simple SMTP relay — but since we have no relay,
    // use nodemailer via dynamic require workaround with the smtp2http pattern.
    // Actually: use the Nodemailer-compatible approach via raw SMTP with node's tls module.
    // Simplest: use node's built-in modules directly.

    try {
      // Use nodemailer dynamically (it's a dep of many packages, may be available)
      // If not, fall back to raw SMTP via fetch to Gmail API
      // We'll use Gmail API via fetch since we have the SMTP credentials
      
      // Raw SMTP via node net + tls
      ...
      console.error("Loyalty digest email error:", err);
  ...
  app.post("/api/loyalty-digest", async (_req, res) => {
    try {
      const result = await sendLoyaltyDigest();
  ...
  // Scheduled trigger endpoint (called by Cloud Scheduler)
  app.get("/api/loyalty-digest-scheduled", async (_req, res) => {
    res.status(200).json({ status: "triggered" }); // respond immediately
