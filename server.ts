  ...
  async function sendLoyaltyDigest(): Promise<{ sent: boolean; entries: number; error?: string }> {
    ...
    if (logs.length === 0) {
      ...
      for (const log of logs) {
        ...
        tableRows += `<tr style="border-bottom:1px solid #eee;"><td style="padding:10px 12px;color:#666;white-space:nowrap;">${time}</td><td style="padding:10px 12px;font-weight:600;color:${actionColor};white-space:nowrap;">${log.action}</td><td style="padding:10px 12px;color:#333;font-size:13px;">${log.details}</td><td style="padding:10px 12px;color:#999;font-size:12px;white-space:nowrap;">${log.userEmail || ""}</td></tr>`;
      }
    }

    const html = `<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;background:#f7f7f7;padding:30px;">
<div style="max-width:700px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
  <div style="background:#1a1a1a;padding:24px 32px;"><p style="margin:0;color:#fff;font-size:20px;font-weight:700;">Cajun Life Cafe</p><p style="margin:4px 0 0;color:#aaa;font-size:13px;">Loyalty Activity Digest — ${dateStr}</p></div>
  <div style="padding:24px 32px 0;"><p style="color:#333;font-size:15px;"><strong>${logs.length} loyalty event${logs.length !== 1 ? "s" : ""}</strong> recorded in the past 24 hours.</p></div>
  <div style="padding:20px 32px;">
    <table width="100%" style="border-collapse:collapse;border:1px solid #eee;">
      <thead><tr style="background:#f9f9f9;"><th style="padding:10px 12px;text-align:left;color:#666;font-size:12px;">Time</th><th style="padding:10px 12px;text-align:left;color:#666;font-size:12px;">Action</th><th style="padding:10px 12px;text-align:left;color:#666;font-size:12px;">Details</th><th style="padding:10px 12px;text-align:left;color:#666;font-size:12px;">Staff</th></tr></thead>
      <tbody>${tableRows}</tbody>
    </table>
  </div>
  <div style="padding:16px 32px 24px;border-top:1px solid #eee;"><p style="color:#aaa;font-size:12px;margin:0;">Auto-generated nightly at midnight ICT. Reply to info@cajunlifecafe.com for disputes.</p></div>
</div>
</body></html>`;
  ...
  // Manual trigger: POST /api/loyalty-digest
  ...
  // Cloud Scheduler trigger: GET /api/loyalty-digest-scheduled
