...
// Hardcoded business info — Google Places removed
const BUSINESS = {
...
const Navbar = ({
  ...
  return (
    <nav
      className={`${
        isDashboard
          ? "sticky top-0 bg-white border-b border-gray-100 py-3 shadow-sm"
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
              ...
              {canAccessDashboard && (
                <Link
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-full font-bold text-sm transition-all ${
                    scrolled
                      ? "bg-cream text-olive hover:bg-olive hover:text-white"
        ...
        <button className="lg:hidden" onClick={() => setIsOpen(!isOpen)}>
          {isOpen ? (
            <X className={scrolled || isDashboard ? "text-ink" : "text-white"} />
          ) : (
            <MenuIcon className={scrolled || isDashboard ? "text-ink" : "text-white"} />
          )}
        </button>
      </div>
          ...
          <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center p-6 text-center">
            ...
            >
              ...
              <div className="mt-12 flex gap-6">
                <a href="#menu" className="terracotta-button px-10 py-4 text-lg">
                  View Menu
                </a>
                <a
                  href="#location"
                  className="bg-white text-ink hover:bg-cream px-10 py-4 rounded-full font-bold text-lg transition-all shadow-xl"
                >
                  Visit Us
                </a>
              </div>
    ...
    <section id="about" className="py-24 px-6 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-4xl md:text-5xl font-display font-bold mb-2 text-terracotta">
            Cajun Life Café
          </h2>
          <h3 className="text-2xl font-serif italic text-olive mb-8">
            Where Healthy Tastes Good
          </h3>
          <p className="text-lg text-gray-600 mb-6 leading-relaxed">
            Our healthy meals are cooked with fresh and clean ingredients that are infused with
            the flavors that embody the traditional taste of Cajun and Creole Cuisine that
            creates a rich bold taste.
          </p>
          <p className="text-lg text-gray-600 mb-8 leading-relaxed">
            Cajun Food is a robust, rustic food found along the bayous of Louisiana, a
            combination of Southern cuisines. Cajun Food is not always spicy, BUT IT ALWAYS HAS
            SPICE. The Cajun "Holy Trinity" of onions, celery and bell pepper contribute to the
            flavor along with spices like salt, pepper and cayenne.
          </p>
        </motion.div>
  ...
  const categories = useMemo(() => {
    const cats =
      categoryList.length > 0
        ...
        : Array.from(new Set<string>(items.map((item) => item.category))).sort();
            ...
            >
              ...
              <div className="space-y-3">
                {item.options.map((opt, oIdx) => (
                  <div
                    key={oIdx}
                    className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-olive/5 shadow-sm"
                  >
    ...
    <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
      ...
      >
        <iframe
          ...
          src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3904.3!2d100.8789!3d12.9088!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3102b5c005a02834%3A0xa178bd5454965996!2sCajun%20Life%20Cafe!5e0!3m2!1sen!2sth!4v1"
        />
      </motion.div>
      ...
      >
        ...
        <div className="mt-12 flex gap-4">
          <a
            ...
            className="bg-cream p-4 rounded-full text-olive hover:bg-olive hover:text-white transition-all"
          >
            <Instagram size={24} />
          </a>
          <a
    ...
    <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-12">
      <div className="col-span-2">
        ...
        <p className="text-gray-400 max-w-sm mb-8">
          Bringing the authentic heart and soul of Louisiana cooking to your neighbourhood.
          Join us for a taste of the bayou.
        </p>
      ...
      <div>
        <h4 className="font-bold mb-6 text-terracotta uppercase tracking-wider text-sm">
          Quick Links
        </h4>
...
function AppContent({ user, setUser }: any) {
  const location = useLocation();
  const isDigitalMenu =
    location.pathname === "/menu" || location.pathname === "/digital-menu";
  const isDashboard =
    location.pathname.startsWith("/dashboard") ||
    location.pathname === "/import" ||
    location.pathname === "/import-custom-meals";
  ...
  const isAdmin = useMemo(() => {
    ...
    return (
      user.email?.toLowerCase() === "info@cajunlifecafe.com" || user.role === "admin"
    );
  }, [user]);
  ...
  const isStaff = useMemo(
    () => isAdmin || ["cashier", "marketing"].includes(user?.role || ""),
    [user, isAdmin]
  );
  ...
  return (
    <div className="min-h-screen">
      ...
      {!isDigitalMenu && !isDashboard && !isEmployee && (
        <Navbar canAccessDashboard={isMarketing} setUser={setUser} />
      )}
      ...
      <Routes>
        ...
        <Route
          path="/dashboard"
          element={
            isAdmin || isMarketing || isStaff ? (
              <DashboardLayout user={user} />
            ) : (
              <div className="pt-32 text-center h-screen bg-cream flex flex-col items-center justify-center gap-4">
                Access Denied. <Auth onUserChange={setUser} />
              </div>
            )
          }
        >
        ...
        />
        <Route
          path="/import-custom-meals"
          element={
            isAdmin || isMarketing ? (
              <BulkCustomMealsImport />
            ) : (
              <div className="pt-32 text-center h-screen bg-cream flex flex-col items-center justify-center gap-4">
                Access Denied. <Auth onUserChange={setUser} />
              </div>
